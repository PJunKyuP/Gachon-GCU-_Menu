...
매일 메뉴를 확인하기 위해 번거롭게 웹사이트에 접속해야 하는 불편함을 해소하고자 기획되었습니다. 학교에 상주하며 학업과 연구, 업무에 매진하시는 학우분들, 교수님들, 그리고 모든 관계 종사자분들께서 매일의 식단을 조금이라도 더 직관적이고 편안하게 확인하실 수 있었으면 하는 따뜻한 바람을 담아 만들었습니다.
...
- **4개 주요 식당 통합 제공**
  ...
  - 체육관(메디컬) 식당
- **스마트하고 세련된 식단 시각화**
  - 곡선형(라운드) 프레임과 투명한 위젯 스타일로 바탕화면과 자연스럽게 어울리는 UI를 제공합니다.
  - 메뉴 카테고리(예: `[점심 A 메뉴]`)를 돋보이는 테마별 컬러로 강조하고 텍스트 중앙 정렬을 적용하여 가독성을 극대화했습니다.
  - 당일 배식 정보가 없을 시 "등록된 메뉴가 없습니다."로 편안하고 일관된 안내를 제공합니다.
  - 날짜 선택 달력(Calendar)에서 휴일과 평일 텍스트 색상을 직관적으로 분리해 누구나 쉽게 날짜를 변경할 수 있습니다.
  - 날짜 이동 버튼(`< 날짜 >`)을 통해 전체 식당 데이터를 동일한 날짜 기준으로 조회할 수 있습니다.
- **다양한 사용자 편의 기능**
  - **진화된 PIP 모드 (스티키 노트)**: 내가 원하는 식당만 포스트잇처럼 화면 구석에 따로 분리해둘 수 있습니다. 깜빡임 없는 독립적인 창 제어 구조가 적용되어 켜져있는 다른 창에 구애받지 않고 원하는 창만 부드럽게 열고 닫을 수 있습니다.
  - **항상 위 창 고정**: 메인 창이나 PIP 창을 다른 창들 위에 항상 떠 있도록 고정할 수 있습니다.
  - **선택적 식당 필터링**: UI 상단의 체크박스를 통해 조회하고 싶은 식당만 깔끔하게 켜고 끌 수 있습니다.
...
2. 사용자 PC의 `%LOCALAPPDATA%\Programs\GachonMenu` 공간으로 파일 복사 및 설치
...
- 설치된 폴더(`%LOCALAPPDATA%\Programs\GachonMenu`) 전체를 깨끗이 삭제합니다.
...
- `assets/images/black_logo.png` / `assets/images/black_logo.ico`: 프로그램 아이콘 리소스
